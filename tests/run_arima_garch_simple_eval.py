...
def main() -> None:
    ...
    y = (df['price'].shift(-4) > 3.0).astype(int).dropna()
    ...
    model_ctor = lambda: ArimaGarchSimpleModel(threshold=3.0)
    ...
    print("===== ARIMA-GARCH SIMPLE EVENT MODEL =====")
    ...
    # ----------------------------
    res.to_csv("arima_garch_simple_eval_results.csv")
    print("\nSaved detailed results to arima_garch_simple_eval_results.csv")
